...
# basic log model
options(scipen=999)
complete_pooling <- glm(asw_correct ~ factor(user_id) + factor(content_id)+
                          month_cut + 
                          q_accu_cut +
                          prior_explain + part, 
...
```{r}
# add user_id and content_id as random effects, varying intercepts
user_pooled1 <- glmer(asw_correct ~ (1|user_id) + (1|content_id) +
                        part + prior_explain +
